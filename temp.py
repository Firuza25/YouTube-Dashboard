...
if add_sidebar == 'Aggregate Metrics':
    ...
    numeric_cols = metric_medians6mo.index

    count = 0
    for i in numeric_cols:
        with columns[count]:
            base = metric_medians12mo[i]
            if base != 0 and not np.isnan(base):
                delta = (metric_medians6mo[i] - base) / base
            else:
                delta = 0
    ...
    for col in df_agg_diff_final.select_dtypes(include=['number']).columns:
        base = median_agg.get(col, None)
        if base and base != 0:
            df_agg_diff_final[col] = df_agg_diff_final[col] / base
        else:
            df_agg_diff_final[col] = np.nan
    
    # форматирование
    df_agg_numeric_lst = df_agg_diff_final.select_dtypes(include=['number']).columns.tolist()
    df_to_pct = {}

    for i in df_agg_numeric_lst:
        if i == "Average % viewed":
            df_to_pct[i] = lambda x: "{:.1%}".format(x/100)   
        elif i == "Engagement_ratio":
...
if add_sidebar == 'Individual Video Analysis':
    ...
    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['median_views'],
                      mode='lines',
                      name='50th percentile', line=dict(color='green', dash ='dash')))
